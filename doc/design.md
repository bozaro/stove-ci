...
### Сборка версии для деплоя
**Общее описание:** при деплое (в случае ручного/автоматического триггера деплоя) или по коммиту (для особых веток,
например `release/*`, `master`) запускается сборка версии для деплоя.

**Подводные камни:**

 - Не нужно собирать версию, если она уже собрана;

 - Иногда нужно собрать версию для конкретного коммита;
 
 - Для каждой ветки нужно иметь свою отдельную историю (к примеру, чтобы можно было смотреть за изменением времени
   сборки).
